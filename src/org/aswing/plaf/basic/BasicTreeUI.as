...
public class BasicTreeUI extends BaseComponentUI implements TreeUI, NodeDimensions, TreeModelListener{
	...
	override public function paint(c:Component, g:Graphics2D, b:IntRectangle):void{
		...
		for(var i:int = 0; i<n; i++){
			...
			if(i < paintingN){
				leaf = treeModel.isLeaf(path.getLastPathComponent());		// 是否叶节点
				clr = treeModel.hasColor(path.getLastPathComponent());
				if(leaf){
					expanded = false;
				}else {
				...
				cell.setTreeCellStatus(tree, selected, expanded, leaf, row, clr);	// （是否选择，是否展开，是否子节点，行, 自定义颜色)
