...
public class DefaultTreeCell extends JLabel implements TreeCell {
	...
	public function setTreeCellStatus(tree : JTree, selected : Boolean, expanded : Boolean, leaf : Boolean, row : int, cellColor : ASColor = null) : void {
		...
		if(selected){
			...
			if(cellColor)
				setForeground(cellColor);		// 自定义颜色
			else
				setForeground(tree.getForeground());	// 默认颜色
		}
