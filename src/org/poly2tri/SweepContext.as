	...
	public class SweepContext {
		...
		protected function initEdges(polyline:Vector.<Point>):void {
			for (var n:uint = 0; n < polyline.length; n++) {
				var p1:Point = polyline[n];
				var p2:Point = polyline[(n + 1) % polyline.length];
				if(p1.equals(p2))
					continue;
				
				this.edge_list.push(new Edge(p1, p2));
		...
		public function meshClean(triangle:Triangle, level:int = 0):void {
			if (level == 0) {
				//for each (var mappedTriangle:Triangle in this.map) Debug.bltrace(mappedTriangle);
			}
