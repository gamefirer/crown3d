package away3d.materials
{
	...
	public class MaterialBase extends NamedAssetBase implements IAsset
	{
		...
		public static var AllMaterialCount : uint = 0;			// 材质数
		/**
		 * An object to contain any extra data
		 */
		public var extra : Object;
		public var bitmapDataUrl : String;		// 用来给编辑器记录贴图url用
		...
		protected var _depthWrite : Boolean = true;
		protected var _depthCompareMode:String = Context3DCompareMode.LESS;

		protected var _depthPass : DepthMapPass;				// 渲染深度图的pass
		protected var _lightMapPass : LightMapPass; 			// 渲染场景光照图的pass
		protected var _distancePass : DistanceMapPass;

		private var _lightPicker : LightPickerBase;
		private var _distanceBasedDepthRender : Boolean;
		
		private var _renderPriority : int = 0;			// 渲染优先级(越大越先渲染)
		...
		public function set renderPriority(value : int) : void
		{
		...
		{
			_depthWrite = value;
			_lightMapPass.depthWrite = value;
			for (var i : int = 0; i < _numPasses; ++i) _passes[i].depthWrite = value;
		}
		...
		public function set depthCompareMode(value : String) : void
		{
			...
			_lightMapPass.depthCompareMode = value;
		...
		*/
		...
		public function set alphaPremultiplied(value : Boolean) : void
		{
			...
			_lightMapPass.alphaPremultiplied = value;
			for (var i : int = 0; i < _numPasses; ++i)
		...
		// 场景lightmap渲染的pass处理
		arcane function activateForLightMap(stage3DProxy : Stage3DProxy, camera : Camera3D, textureRatioX : Number = 1, textureRatioY : Number = 1) : void
		{
			_lightMapPass.activate(stage3DProxy, camera, textureRatioX, textureRatioY);
		}
		
		arcane function deactivateForLightMap(stage3DProxy : Stage3DProxy) : void
		{
			_lightMapPass.deactivate(stage3DProxy);
		}
		
		arcane function renderLightMap(renderable : IRenderable, stage3DProxy : Stage3DProxy, camera : Camera3D) : void
		{
			if (renderable.animator)
				_lightMapPass.updateAnimationState(renderable, stage3DProxy);
			
			_lightMapPass.render(renderable, stage3DProxy, camera, _lightPicker);
		}
			...
			for(i=0; i<_owners.length; i++)
			{
				...
				if (owner.animator) {
					if (_animationSet && owner.animator.animationSet != _animationSet) {
						...
						_animationSet = owner.animator.animationSet;
						for (var passi : int = 0; passi < _numPasses; ++passi)
							_passes[passi].animationSet = _animationSet;
						_depthPass.animationSet = _animationSet;
						_distancePass.animationSet = _animationSet;
					}
				}
