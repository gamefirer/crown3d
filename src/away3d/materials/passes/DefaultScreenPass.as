...
//		private var _uvTransformData : Vector.<Number>;
		...
		private var _colorMethod : BasicColorMethod;					// 顶点色的渲染方法
		private var _colorMethodVO : MethodVO;
		private var _normalMethod : BasicNormalMethod;					// normalmap的渲染方法
		private var _normalMethodVO : MethodVO;
		private var _ambientMethod : BasicAmbientMethod;				// 环境光(必须)
		private var _ambientMethodVO : MethodVO;
		private var _shadowMethod : ShadowMapMethodBase;				// shadowmap的渲染方法
		private var _shadowMethodVO : MethodVO;
		private var _diffuseMethod : BasicDiffuseMethod;				// diffuse(必须)
		private var _diffuseMethodVO : MethodVO;
		private var _specularMethod : BasicSpecularMethod;				// 镜面反射
		private var _specularMethodVO : MethodVO;
		private var _lightMapMethod : SceneLightMapMethod;				// 场景Lightmap的渲染方法
		private var _lightMapMethodVO : MethodVO;
		...
		private var _colorDependencies : uint;				// 对顶点数据的需求

		// registers
		protected var _colorBufferIndex : int;			// 存放顶点色寄存器的index
		...
		// 变量寄存器
		private var _colorVarying : ShaderRegisterElement;
		...
		private var _userVertexReg : ShaderRegisterElement;
		private var _userVertexDataIndex : int;
...
//		private var _commonsData : Vector.<Number> = Vector.<Number>([.5, 0, 0, 1]);
		...
		private var _zbais : Number = 0;
		
		protected var _projectedTargetRegister : String;
		protected var _vertexRenderDataRegister : String;	// 顶点渲染(vertex shader)用的外部数据(x:zbais, y: ,z: ,w:)
		...
		private function init() : void
		{
			...
			_colorMethod = new BasicColorMethod();
			...
			_colorMethod.addEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
			...
			_colorMethodVO = _colorMethod.createMethodVO();
		}
		...
		{
			_zbais = value;
			_vertexConstantData[_userVertexDataIndex] = _zbais * 0.002;		// 根据camera距离设置一个合适的值
		}
		...
		override public function dispose() : void
		{
			...
			if (_lightMapMethod) {		// 释放
				_lightMapMethod.removeEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
				_lightMapMethod.dispose();
			}
			_ambientMethod.removeEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
			_ambientMethod.dispose();
			if(_colorMethod) {
				_colorMethod.removeEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
...
//			_methods = null;		// 不应=null
		...
		public function get colorMethod() : BasicColorMethod
		{
			return _colorMethod;
		}
		
		public function set colorMethod(value : BasicColorMethod) : void
		{
			_colorMethod.removeEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
			value.copyFrom(_colorMethod);
			_colorMethod = value;
			_colorMethod.addEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
			_colorMethodVO = _colorMethod.createMethodVO();
			invalidateShaderProgram();
		...
		public function set lightMapMethod(value : SceneLightMapMethod) : void
		{
			if (_lightMapMethod) _lightMapMethod.removeEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
			_lightMapMethod = value;
			if (_lightMapMethod) {
				_lightMapMethod.addEventListener(ShadingMethodEvent.SHADER_INVALIDATED, onShaderInvalidated);
		...
		private function getProjectionCode(positionRegister : String, projectionRegister : String, normalRegister : String) : String
		{
			...
			// zbais处理 
//			if(_userVertexReg)		// vertex shader 用户设定数据
//			{
				// z bais
//				code += "sub op.z, vt7.z, " + vertexRenderDataRegister + ".x\n";		
//				code += "mul op.z, vt7.z, " + vertexRenderDataRegister + ".y\n";
				code += "sub vt7.x, vt7.w, vt7.z\n" +									// w-z
					"mul vt7.x, vt7.x, " + _userVertexReg + ".x\n" +		// 0.01*(w-z)
					"sub op.z, vt7.z, vt7.x\n";
//			}
		...
		override arcane function activate(stage3DProxy : Stage3DProxy, camera : Camera3D, textureRatioX : Number, textureRatioY : Number) : void
		{
			...
			if (_colorMethodVO) _colorMethod.activate(_colorMethodVO, stage3DProxy);
			if (_shadowMethod) _shadowMethod.activate(_shadowMethodVO, stage3DProxy);
			if (_lightMapMethod) _lightMapMethod.activate(_lightMapMethodVO, stage3DProxy);
		...
		arcane override function deactivate(stage3DProxy : Stage3DProxy) : void
		{
			...
			if (_colorMethodVO) _colorMethod.deactivate(_colorMethodVO, stage3DProxy);
			...
			if (_lightMapMethod) _lightMapMethod.deactivate(_lightMapMethodVO, stage3DProxy);
		...
		{
			Profiler.start("DefaultScreenPass.render");
			var i : uint;
			var context : Context3D = stage3DProxy._context3D;
			if (_colorBufferIndex >= 0) stage3DProxy.setSimpleVertexBuffer(_colorBufferIndex, renderable.getVertexColorBuffer(stage3DProxy), Context3DVertexBufferFormat.FLOAT_4, renderable.colorBufferOffset);
			...
			if (_colorMethodVO) _colorMethod.setRenderState(_colorMethodVO, renderable, stage3DProxy, camera);
			...
			Profiler.end("DefaultScreenPass.render");
		...
		arcane override function invalidateShaderProgram(updateMaterial : Boolean = true) : void
		{
			...
			if (_colorMethod) addPasses(_colorMethod.passes);
			if (_shadowMethod) addPasses(_shadowMethod.passes);
			if (_lightMapMethod) addPasses(_lightMapMethod.passes);
		...
		private function reset() : void
		{
			...
			_vertexConstantIndex = _registerCache.vertexConstantOffset = 5;		// 常量寄存器已经使用5个(mvp 4个，radio 1个)
		...
		private function updateMethodConstants() : void
		{
			...
			if (_colorMethod) _colorMethod.initConstants(_colorMethodVO);
			...
			if (_lightMapMethod) _lightMapMethod.initConstants(_lightMapMethodVO);
		...
		{
			// vs公共数据
			_vertexConstantData[_userVertexDataIndex] = _zbais * 0.002;
			_vertexConstantData[_userVertexDataIndex + 1] = 0;
			_vertexConstantData[_userVertexDataIndex + 2] = 0;
			_vertexConstantData[_userVertexDataIndex + 3] = 0;
		...
		{
			createCommons();
			calculateDependencies();			// 计算变量寄存器需求
			...
			if (_colorDependencies > 0) compileColorCode();
			...
			if (_colorMethod) updateMethodRegisters(_colorMethod);
			if (_shadowMethod) updateMethodRegisters(_shadowMethod);
			if (_lightMapMethod) updateMethodRegisters(_lightMapMethod);
		...
		{
			method.colorFragmentReg = _colorVarying;
		...
		private function calculateDependencies() : void
		{
			...
			if (_lightMapMethod) setupAndCountMethodDependencies(_lightMapMethod, _lightMapMethodVO);
			...
			if (_colorMethod) setupAndCountMethodDependencies(_colorMethod, _colorMethodVO);
		...
		private function countDependencies(methodVO : MethodVO) : void
		{
			...
			if (methodVO.needsColor) ++_colorDependencies;
...
//				_registerCache.removeVertexTempUsage(_globalPositionVertexReg);
		...
		private function compileColorCode() : void
		{
			var colorAttributeReg : ShaderRegisterElement = _registerCache.getFreeVertexAttribute();
			
			_colorVarying = _registerCache.getFreeVarying();
			_colorBufferIndex = colorAttributeReg.index;
			
			// 顶点色
			_vertexCode += "mov " + _colorVarying + ", " + colorAttributeReg + "\n";
		}
		...
		private function createCommons() : void
		{
			...
			_userVertexReg = _registerCache.getFreeVertexConstant();
			_userVertexDataIndex = (_userVertexReg.index - _registerCache.vertexConstantOffset)*4;
		}
		...
		private function compileLightingCode() : void
		{
			...
			var lightmapReg : ShaderRegisterElement;			// lightmap用寄存器
			...
			// vertex color 顶点色
			if (_colorMethod) {
				_vertexCode += _colorMethod.getVertexCode(_colorMethodVO, _registerCache);
				_fragmentCode += _colorMethod.getFragmentCode(_colorMethodVO, _registerCache, _shadedTargetReg);
			}
			
			// 场景lightmap处理
			if (_lightMapMethod) {
				_vertexCode += _lightMapMethod.getVertexCode(_lightMapMethodVO, _registerCache);
				
				lightmapReg = _registerCache.getFreeFragmentVectorTemp();
				_registerCache.addFragmentTempUsages(lightmapReg, 1);
				
				_diffuseMethod.lightmapRegister = lightmapReg;		// use lightmapReg
				_fragmentCode += _lightMapMethod.getFragmentCode(_lightMapMethodVO, _registerCache, lightmapReg);	
			}
			...
			if(lightmapReg)
				_registerCache.removeFragmentTempUsage(lightmapReg);
			if (shadowReg && _normalDependencies > 0)
				_registerCache.removeFragmentTempUsage(shadowReg);
		}
