...
//	public class PathAnimator extends EventDispatcher
	public class PathAnimator	extends AnimatorBase		// 改PathAnimator继承于AnimatorBase,所有动画用统一的实现方法
	{
		...
		private var _targetBasePosition : Vector3D;			// target的原始位置
		
		private var _pathMaker : PathMaker;
		private var _totalDuration : uint = 0;
		public var isLoop:Boolean = true;		// 循环路径
		 ...
		 */
//		function PathAnimator(path:IPath = null, target:Object3D = null, offset:Vector3D = null, alignToPath:Boolean = false, lookAtTarget:Object3D = null, rotations:Vector.<Vector3D> = null)
		function PathAnimator(pathMaker:PathMaker, target:Object3D = null, offset:Vector3D = null, alignToPath:Boolean = false, lookAtTarget:Object3D = null, rotations:Vector.<Vector3D> = null)
		{
			super(null);
		...
		public function set pathMaker(maker:PathMaker):void
		{
			_pathMaker = maker;
			_path = _pathMaker.makePath();
			durationTime = _pathMaker.duration;
		}
		
		public function get pathMaker():PathMaker {return _pathMaker;}				
		...
		public function set durationTime(durationTime : uint) : void {_totalDuration = durationTime;}
		
		// 更新动画
		protected override function updateDeltaTime(dt:Number) : void
		{
			if(!_target) return;
			if(_totalDuration <= 0) return;
			
			//			Profiler.start("PathAnimator");
			
			_absoluteTime += dt;
			
			var w:Number = _absoluteTime / _totalDuration;
			if(isLoop)
			{
				if(w < 0) w = 0;
				if(w > 1) w = w % 1;
		...
		{
			if(_target && _targetBasePosition)
			{
				_target.position = _targetBasePosition;
			}
			_target = object3d;
			if(_target)
		...
		private function updateObjectPosition(rotate:Boolean = false):void
		{
			...
			if(!global)
			{
				_position.x += _targetBasePosition.x;
				_position.y += _targetBasePosition.y;
				_position.z += _targetBasePosition.z;
			}
