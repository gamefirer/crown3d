package away3d.lights
{
	...
	public class LightBase extends Entity
	{
		...
		private var _castsShadows : Boolean;			// 是否生成阴影贴图(shadowmap)
		private var _castsLightMap : Boolean = false;		// 是否生成灯光贴图

		private var _shadowMapper : ShadowMapperBase;	// Shadowmap渲染器
		private var _lightMapper : AreaLightMapper;		// 灯光贴图渲染器
		...
		public function set castsLightMap(value : Boolean) : void
		{
			if (_castsLightMap == value) return;
			_castsLightMap = value;
		...
		{
			return new AreaLightMapper(this);		// 使用跟随灯的阴影产生器
		}
