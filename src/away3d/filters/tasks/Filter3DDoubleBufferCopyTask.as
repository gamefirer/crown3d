package away3d.filters.tasks
{
	...
	public class Filter3DDoubleBufferCopyTask extends Filter3DTaskBase
	{
		...
		override protected function updateTextures(stage : Stage3DProxy) : void
		{
			...
			if (_secondaryInputTexture) Context3DProxy.disposeTexture(_secondaryInputTexture); // _secondaryInputTexture.dispose();

			_secondaryInputTexture = Context3DProxy.createTexture(_textureWidth >> _textureScale, _textureHeight >> _textureScale, Context3DTextureFormat.BGRA, true);
			var dummy : BitmapData = new BitmapData(_textureWidth >> _textureScale, _textureHeight >> _textureScale, false, 0);
			Context3DProxy.uploadTextureFromBitmapData(_mainInputTexture, dummy);
//			_mainInputTexture.uploadFromBitmapData(dummy);
			Context3DProxy.uploadTextureFromBitmapData(_secondaryInputTexture, dummy);
