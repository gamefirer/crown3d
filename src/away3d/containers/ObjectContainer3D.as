package away3d.containers
{
	...
	{
		public static var Object3DNumber : int = 0;			// 空间对象数
		/** @private */
		...
		*/
		...
		public function update(curTime : uint, deltaTime : uint):void
		{
			for (var i : uint = 0; i <  _children.length; ++i)
				_children[i].update(curTime, deltaTime);
		}
		
		/**
		 *	动画相关 
		 */
		private var _lastUpdateAnimationTime : uint = 0;
		override public function updateAnimation(curTime : uint, deltaTime : uint) : void
		{
			if(_lastUpdateAnimationTime == curTime)
				return;
			_lastUpdateAnimationTime = curTime;
			
			if(parent)
				parent.updateAnimation(curTime, deltaTime);		// 更新父节点的动画
			
			super.updateAnimation(curTime, deltaTime);
		}
