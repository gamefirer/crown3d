	...
	public class View3D extends Sprite
	{
		...
		public function set filters3d(value : Array) : void
		{
			...
			if (_filter3DRenderer) {
				...
				if (_depthRender) {
					Context3DProxy.disposeTexture(_depthRender); // _depthRender.dispose();
					_depthRender = null;
				}
			}
		}
		...
		public function render() : void
		{
			...
			// 触发鼠标事件, fire collected mouse events
			_mouse3DManager.fireMouseEvents();
			
			Profiler.end("View3D.render");
		}
		...
		protected function renderSceneDepth(entityCollector : EntityCollector) : void
		{
			if (_depthTextureInvalid || !_depthRender) 
				initDepthTexture(_stage3DProxy._context3D);		// 创建深度贴图
		...
		private function initDepthTexture(context : Context3D) : void
		{
			...
			if (_depthRender) 
				Context3DProxy.disposeTexture(_depthRender);

			_depthRender = Context3DProxy.createTexture(_rttBufferManager.textureWidth, _rttBufferManager.textureHeight, Context3DTextureFormat.BGRA, true);
		}
