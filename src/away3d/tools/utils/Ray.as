package away3d.tools.utils
{
	...
	public class Ray{
		...
		private var _tmp:Vector3D = new Vector3D();				// 计算用暂存
		private var _tmpDir:Vector3D = new Vector3D();
		...
		*/
		...
		// 该函数不同于getRayToTriangleIntersection, 它不改变Ray的_dir和_orig
		public function getIntersectWithTriangle(p0:Vector3D, p1:Vector3D, v0:Vector3D, v1:Vector3D, v2:Vector3D):Vector3D
		{
			_tu.x = v1.x - v0.x;
			_tu.y = v1.y - v0.y;
			_tu.z = v1.z - v0.z;
			...
			var a:Number = _npn.x * _tmp.x + _npn.y * _tmp.y + _npn.z * _tmp.z;
			
			if (a ==0)
				return null;
			
			var b:Number = _pn.x * _tmpDir.x + _pn.y * _tmpDir.y + _pn.z * _tmpDir.z;
			var r:Number = a / b;
			
			if (r < 0 || r > 1)
				return null;
			
			_intersect.x = p0.x+(_tmpDir.x*r);
			_intersect.y = p0.y+(_tmpDir.y*r);
			_intersect.z = p0.z+(_tmpDir.z*r);
