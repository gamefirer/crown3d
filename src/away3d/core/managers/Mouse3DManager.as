package away3d.core.managers
{
	...
	public class Mouse3DManager
	{
		...
		private var _mousePicker : IPicker = PickingType.RAYCAST_BEST_HIT; // PickingType.RAYCAST_FIRST_ENCOUNTERED;
		...
		private function onMouseOut(event : MouseEvent) : Boolean
		{
			_activeView = null;
		...
		private function onMouseOver(event : MouseEvent) : Boolean
		{
		...
		private function onClick(event : MouseEvent) : Boolean
		{
		...
		private function onDoubleClick(event : MouseEvent) : Boolean
		{
...
//			view.addEventListener(MouseEvent.MOUSE_OUT, onMouseOut);
			BlInputManager.addMouseClickHandler(onClick);
			BlInputManager.addMouseDoubleHandler(onDoubleClick);
			...
			BlInputManager.addMouseOverHandler(onMouseOver);
			BlInputManager.addMouseOutHandler(onMouseOut);
			
			_activeView = view;			// 无 MouseOver
		}
...
//			view.removeEventListener(MouseEvent.MOUSE_OUT, onMouseOut);
			BlInputManager.removeMouseClickHandler(onClick);
			BlInputManager.removeMouseDoubleHandler(onDoubleClick);
			BlInputManager.removeMouseDownHandler(onMouseDown);
			BlInputManager.removeMouseMoveHandler(onMouseMove);
			BlInputManager.removeMouseUpHandler(onMouseUp);
			BlInputManager.removeMouseWheelHandler(onMouseWheel);
			BlInputManager.removeMouseOverHandler(onMouseOver);
			BlInputManager.removeMouseOutHandler(onMouseOut);
		}
