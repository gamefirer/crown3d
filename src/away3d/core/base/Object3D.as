package away3d.core.base
{
	...
	import away3d.animators.RenderAnimatorBase;
	...
	public class Object3D extends NamedAssetBase
	{
		...
		private var _disposeEvent:Object3DEvent;
		...
		private var _listenToDispose : Boolean

		// Render动画(只在渲染时才会更新的动画)
		protected var _renderAnimators : Vector.<RenderAnimatorBase> = new Vector.<RenderAnimatorBase>;		// 渲染动画控制器
		private var _scaleAnimator : ScaleAnimator;	// 缩放动画
		private var _rotateAnimator : RotateAnimator;	// 旋转动画
		// Frame动画(每帧都会更新的动画)
		protected var _frameAnimators : Vector.<AnimatorBase> = new Vector.<AnimatorBase>;				// 帧动画控制器
		private var _pathAnimator : PathAnimator;		// 路径动画(Path动画继承于AnimatorBase，采用onFrameEnter的方法更新，鉴于其特殊的动画形式)
		...
		override public function addEventListener(type : String, listener : Function, useCapture : Boolean = false, priority : int = 0, useWeakReference : Boolean = false) : void
		{
			...
			switch (type) {
				...
				case Object3DEvent.SCALE_CHANGED:
					_listenToRotationChanged = true;
					break;
				case Object3DEvent.DISPOSE:
					_listenToDispose = true;
					break;
			}
		}
		...
		public function scaleXYZ(val:Number) : void
		{
			if(_scaleX == val && _scaleY == val && _scaleZ == val)
				return;
		...
		{
			if (!_disposeEvent)
				_disposeEvent = new Object3DEvent(Object3DEvent.DISPOSE, this);
			
			dispatchEvent(_disposeEvent);
		}
		...
		protected function updateTransform() : void
		{
			...
			if(rotateAnimator)
			{
				_rot.x = _rotationX + rotateAnimator.ani_RotX;
				_rot.y = _rotationY + rotateAnimator.ani_RotY;
				_rot.z = _rotationZ + rotateAnimator.ani_RotZ;
			}
			else
			{
				_rot.x = _rotationX;
			...
			if(scaleAnimator)
			{
				_sca.x = _scaleX * scaleAnimator.ani_ScaleX;
				_sca.y = _scaleY * scaleAnimator.ani_ScaleY;
				_sca.z = _scaleZ * scaleAnimator.ani_ScaleZ;
			}
			else
			{
				_sca.x = _scaleX;
		...
		{
			for(var ai:int=0; ai<_renderAnimators.length; ai++)
			{
				_renderAnimators[ai].updateAnimation(curTime, deltaTime);
			}
			
			
		}
		
		protected function addRenderAnimator(animator : RenderAnimatorBase) : void
		{
			_renderAnimators.push(animator);
		}
		
		protected function removeRenderAnimator(animator : RenderAnimatorBase) : void
		{
			_renderAnimators.splice(_renderAnimators.indexOf(animator), 1);
		}
		
		protected function addFrameAnimator(animator : AnimatorBase) : void
		{
			_frameAnimators.push(animator);
		}
		
		protected function removeFrameAnimator(animator : AnimatorBase) : void
		{
			_frameAnimators.splice(_frameAnimators.indexOf(animator), 1);
		}
		
		public function set pathAnimator(animator : PathAnimator) : void
		{
			if(_pathAnimator)
			{
				removeFrameAnimator(_pathAnimator);
				_pathAnimator.target = null;
			}
			_pathAnimator = animator;
			
			if(!_pathAnimator)
				return;
			
			_pathAnimator.target = this;
			addFrameAnimator(_pathAnimator);
		}
		
		public function get pathAnimator() : PathAnimator {return _pathAnimator;}
		
		public function set scaleAnimator(animator : ScaleAnimator) : void
		{
			if(_scaleAnimator)
			{
				removeRenderAnimator(_scaleAnimator);
				_scaleAnimator.target = null;
			}
			_scaleAnimator = animator;
			
			if(!_scaleAnimator)
				return;
			
			_scaleAnimator.target = this;
			addRenderAnimator(_scaleAnimator);
		}
		
		public function get scaleAnimator() : ScaleAnimator {return _scaleAnimator;}
		
		public function set rotateAnimator(animator : RotateAnimator) : void
		{
			if(_rotateAnimator)
			{
				removeRenderAnimator(_rotateAnimator);
				_rotateAnimator.target = null;
			}
			_rotateAnimator = animator;
			
			if(!_rotateAnimator)
				return;
			
			_rotateAnimator.target = this;
			addRenderAnimator(_rotateAnimator);
		}
		
		public function get rotateAnimator() : RotateAnimator {return _rotateAnimator;}
		
		
		public function setAnimatorsTime(pastTime : Number) : void
		{
			for(var ai:int=0; ai<_renderAnimators.length; ai++)
			{
				_renderAnimators[ai].reset(pastTime);
			}
		}
		
		public function playAllAnimators() : void
		{
			var ai:int;
			for(ai=0; ai<_renderAnimators.length; ai++)
			{
				_renderAnimators[ai].playDefault();
			}
			for(ai=0; ai<_frameAnimators.length; ai++)
			{
				_frameAnimators[ai].start();
			}
		}
		
		public function stopAllAnimators() : void
		{
			var ai:int;
			for(ai=0; ai<_renderAnimators.length; ai++)
			{
				_renderAnimators[ai].stop();
			}
			for(ai=0; ai<_frameAnimators.length; ai++)
			{
				_frameAnimators[ai].stop();
			}
		}
	}
}
