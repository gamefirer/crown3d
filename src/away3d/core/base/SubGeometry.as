package away3d.core.base
{
	...
	public class SubGeometry
	{
		...
		protected var _verticesColor : Vector.<Number>;
		...
		protected var _vertexColorBuffer : Vector.<VertexBuffer3D> = new Vector.<VertexBuffer3D>(8);	// 顶点色buffer
		...
		private var _autoDeriveVertexColors : Boolean = true;				// 自动生成顶点色buffer
		private var _autoGenerateUVs : Boolean = false;
		private var _autoDeriveVertexNormals : Boolean = true;			// 自动计算normal
		private var _autoDeriveVertexTangents : Boolean = true;			// 自动计算切线
		...
		*/
		...
		public function getVertexColorBuffer(stage3DProxy : Stage3DProxy) : VertexBuffer3D
		{
			...
			if (_autoDeriveVertexColors && _vertexColorsDirty)
				updateVertexColors();
			
			if (_vertexColorBufferContext[contextIndex] != context || !_vertexColorBuffer[contextIndex])
			{
				// 释放旧vb
				if(_vertexColorBuffer[contextIndex])
				{
					Context3DProxy.disposeVertexBuffer(_vertexColorBuffer[contextIndex]);
				}
				// 创建新vb
		...
		public function getSecondaryUVBuffer(stage3DProxy : Stage3DProxy) : VertexBuffer3D
		{
			...
			{
				if(_secondaryUvBuffer[contextIndex])
				{
		...
		public function disposeGpuBuffer() : void
		{
			disposeAllVertexBuffers();
			disposeIndexBuffers(_indexBuffer);
		}
		...
		{
			if( _autoDeriveVertexColors && _vertexColorsDirty) updateVertexColors();
			return _verticesColor;
		}
		
		public function hasVertexColorData() : Boolean
		{
			return _verticesColor != null;
		}
		
		public function updateVertexColorData(vertexColor : Vector.<Number>) : void
		{
		...
		protected function disposeVertexBuffers(buffers : Vector.<VertexBuffer3D>) : void
		{
			for (var i : int = 0; i < 8; ++i) {
				if (buffers[i]) {
					Context3DProxy.disposeVertexBuffer(buffers[i]);
//					buffers[i].dispose();
					buffers[i] = null;
				}
			}
		...
		protected function disposeIndexBuffers(buffers : Vector.<IndexBuffer3D>) : void
		{
			for (var i : int = 0; i < 8; ++i) {
				if (buffers[i]) {
					Context3DProxy.disposeIndexBuffer(buffers[i]);
//					buffers[i].dispose();
					buffers[i] = null;
				}
			}
		...
		private function updateVertexColors() : void
		{
			...
			var lenC : uint = lenV/3*4;
			
			var ci:uint=0;
			if(_verticesColor)
			{
				while(ci<lenC) _verticesColor[ci++] = 1.0;
			}
			else
			{
				_verticesColor = new Vector.<Number>(lenC, true);
				
				for(ci=0; ci<lenC; ci++)
				{
					_verticesColor[ci] = 1.0;
				}
			}
		...
		protected function disposeForStage3D(stage3DProxy : Stage3DProxy) : void
		{
			...
			if (_vertexColorBuffer[index]) {
				_vertexColorBuffer[index].dispose();
				_vertexColorBuffer[index] = null;
			}
			if (_uvBuffer[index]) {
