package away3d.entities
{
	...
	{
		public static var MeshNumber : int = 0;			// mesh数
		...
		private var _uvAnimator : BlUVAnimator;			// UV动画
		private var _colorAnimator : ColorAnimator;	// 颜色动画
...
//				}
				if(subMesh._material)
					subMesh._material.reOwner();
			}
		...
		public function set material(value : MaterialBase) : void
		{
			...
			// 颜色动画设置
			if(_colorAnimator && _material)
				_colorAnimator.setTargetMesh(this);
		}
		...
		*/
		...
		private function addSubMesh(subGeometry : SubGeometry) : void
		{
			...
			// uv动画设置
			if(_uvAnimator)
				_uvAnimator.setTargetMesh(subMeshes[0]);
		}
		...
		public function set uvAnimator(uvAnimator : BlUVAnimator) : void
		{
			if(_uvAnimator)
			{
				_uvAnimator.setTargetMesh(null);
				removeRenderAnimator(_uvAnimator);
			}
			_uvAnimator = uvAnimator;
			...
			addRenderAnimator(_uvAnimator);
		}
		
		public function get uvAnimator() : BlUVAnimator {return _uvAnimator;}
		
		public function set colorAnimator(colorAnimator : ColorAnimator) : void
		{
			if(_colorAnimator)
			{
				removeRenderAnimator(_colorAnimator)
				_colorAnimator.setTargetMesh(null);
			}
			...
			addRenderAnimator(_colorAnimator);
		}
		
		public function get colorAnimator() : ColorAnimator {return _colorAnimator;}
	}
}
