package away3d.entities
{
	...
	{
		static public var Normal_Layer 			: uint = 0x01;		// 默认渲染层
		static public var Decal_Layer 			: uint = 0x02;		// 地表贴花层
		static public var Character_Layer 		: uint = 0x04;		// 角色渲染层
		static public var Effect_Layer 			: uint = 0x08;		// 特效渲染层
		static public var SceneLight_Layer 		: uint = 0x10;		// 贴图灯渲染层
		static public var Project_Layer			: uint = 0x20;		// 投影层
		static public var Editor_Layer			: uint = 0x40;		// 编辑器层
		static public var All_Layer				: uint = 0xff;
		
		public static var EntityNumber : int = 0;			// entity数
		...
		public var isRendering : Boolean = false;			// 该对象当前帧是否被渲染
		...
		private var _renderLayer : uint = Normal_Layer;			// 渲染层
		
		public function set renderLayer(layer:uint) : void {_renderLayer = layer;}
		public function get renderLayer() : uint {return _renderLayer;}
		...
		public function preRender(traverser : EntityCollector) : void	// 渲染前,被调用
		{
			isRendering = true;
		}
		...
		override public function update(curTime : uint, deltaTime : uint):void
		{
			if(isRendering)
			{	// 被渲染才调用的动画
				updateAnimation(curTime, deltaTime);
			}
			
			super.update(curTime, deltaTime);
			isRendering = false;		// 清除渲染标志
		}
