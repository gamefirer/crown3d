	...
	public class TextureProxyBase extends NamedAssetBase implements IAsset
	{
		...
		public function getTextureForStage3D(stage3DProxy : Stage3DProxy) : TextureBase
		{
			...
			if (!tex || _dirty[contextIndex] != context) 
			{
				...
				{	// 释放旧贴图对象
					Context3DProxy.disposeTexture(_textures[contextIndex]);
					_textures[contextIndex] = tex = null;
				}
				_textures[contextIndex] = tex = createTexture(context);		// 创建贴图
				_dirty[contextIndex] = context;
				uploadContent(tex);			// 更新贴图
			}
		...
		protected function invalidateSize() : void
		{
			...
			for (var i : int = 0; i < 8; ++i) {
				tex = _textures[i];
				if (tex) {
					Context3DProxy.disposeTexture(tex); //	tex.dispose();
					_textures[i] = null;
					_dirty[i] = null;
				}
			}
		...
		public function dispose() : void
		{
			for (var i : int = 0; i < 8; ++i)
				if (_textures[i]) 
					Context3DProxy.disposeTexture(_textures[i]); // _textures[i].dispose();
		}
	}
}