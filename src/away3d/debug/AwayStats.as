package away3d.debug
{
	...
	public class AwayStats extends Sprite
	{
		...
		private var _particleNum : uint;	// 粒子数
		private var _drawNum : uint;		// drawcall数
		private var _drawUINum : uint;		// 3d ui drawcall数
		private var _numEntities : uint;	// 渲染entity数
		private var _numOpaques : uint;	// 渲染不透明物体数
		private var _numBlended : uint;	// 渲染透明物体数
		private var _numLights : uint;		// 渲染灯数
		private var _numEditor : uint;		// 渲染编辑器物体
		private var _skelCalcNum : uint;	// 骨骼动画计算次数
		...
		private var _particle_tf : TextField;			// 显示粒子数
		private var _drawCall_tf : TextField;			// draw的次数
		private var _upload_tf : TextField;			// upload次数
		private var _entity_tf : TextField;			// 渲染entity数
		private var _skelCalc_tf : TextField;			// 骨骼动画计算次数
		...
		private static const _MID_HEIGHT : Number = 120;
		private static const _MAX_HEIGHT : Number = 85 + _MID_HEIGHT;
		private static const _MIN_HEIGHT : Number = 51 + _MID_HEIGHT;
		private static const _UPPER_Y : Number = -1;
		private static const _MID_Y : Number = 9;
		private static const _MID2_Y : Number = 19;
		private static const _MID3_Y : Number = 29;
		private static const _MID4_Y : Number = 39;
		private static const _MID5_Y : Number = 49;
		private static const _MID6_Y : Number = 59;
		private static const _LOWER_Y : Number = 69;
		private static const _DIAG_HEIGHT : Number = _MAX_HEIGHT - 50 - _MID_HEIGHT;	// 线表框高;
		private static const _BOTTOM_BAR_HEIGHT : Number = _MAX_HEIGHT - _DIAG_HEIGHT - 10;		// 信息栏高
		...
		private function _initBottomBar() : void
		{
			...
			var drawCall_label_tf : TextField;
			var particle_label_tf : TextField;
			...
			// 粒子数
			particle_label_tf = new TextField;
			particle_label_tf.defaultTextFormat = _label_format;
			particle_label_tf.autoSize = TextFieldAutoSize.LEFT;
			particle_label_tf.text = 'particle';
			particle_label_tf.x = 10;
			particle_label_tf.y = _MID2_Y;
			particle_label_tf.selectable = false;
			particle_label_tf.mouseEnabled = false;
			_btm_bar.addChild(particle_label_tf);
			
			_particle_tf = new TextField;
			_particle_tf.defaultTextFormat = _data_format;
			_particle_tf.autoSize = TextFieldAutoSize.LEFT;
			_particle_tf.x = particle_label_tf.x + 45;
			_particle_tf.y = _MID2_Y;
			_particle_tf.selectable = false;
			_particle_tf.mouseEnabled = false;
			_btm_bar.addChild(_particle_tf);
			
			// drawcall数
			drawCall_label_tf = new TextField;
			drawCall_label_tf.defaultTextFormat = _label_format;
			drawCall_label_tf.autoSize = TextFieldAutoSize.LEFT;
			drawCall_label_tf.text = 'DrawCall:';
			drawCall_label_tf.x = 10;
			drawCall_label_tf.y = _MID3_Y;
			drawCall_label_tf.selectable = false;
			drawCall_label_tf.mouseEnabled = false;
			_btm_bar.addChild(drawCall_label_tf);
			
			_drawCall_tf = new TextField;
			_drawCall_tf.defaultTextFormat = _data_format;
			_drawCall_tf.autoSize = TextFieldAutoSize.LEFT;
			_drawCall_tf.x = poly_label_tf.x + 45;
			_drawCall_tf.y = _MID3_Y;
			_drawCall_tf.selectable = false;
			_drawCall_tf.mouseEnabled = false;
			_btm_bar.addChild(_drawCall_tf);
			
			// upload数
			var upload_label_tf : TextField = new TextField;
			upload_label_tf.defaultTextFormat = _label_format;
			upload_label_tf.autoSize = TextFieldAutoSize.LEFT;
			upload_label_tf.text = 'Upload:';
			upload_label_tf.x = 10;
			upload_label_tf.y = _MID4_Y;
			upload_label_tf.selectable = false;
			upload_label_tf.mouseEnabled = false;
			_btm_bar.addChild(upload_label_tf);
			
			_upload_tf = new TextField;
			_upload_tf.defaultTextFormat = _data_format;
			_upload_tf.autoSize = TextFieldAutoSize.LEFT;
			_upload_tf.x = upload_label_tf.x + 45;
			_upload_tf.y = _MID4_Y;
			_upload_tf.selectable = false;
			_upload_tf.mouseEnabled = false;
			_btm_bar.addChild(_upload_tf);
			
			// 渲染对象数
			var entity_label_tf : TextField = new TextField;
			entity_label_tf.defaultTextFormat = _label_format;
			entity_label_tf.autoSize = TextFieldAutoSize.LEFT;
			entity_label_tf.text = 'eoble:';
			entity_label_tf.x = 10;
			entity_label_tf.y = _MID5_Y;
			entity_label_tf.selectable = false;
			entity_label_tf.mouseEnabled = false;
			_btm_bar.addChild(entity_label_tf);
			
			_entity_tf = new TextField;
			_entity_tf.defaultTextFormat = _data_format;
			_entity_tf.autoSize = TextFieldAutoSize.LEFT;
			_entity_tf.x = entity_label_tf.x + 45;
			_entity_tf.y = _MID5_Y;
			_entity_tf.selectable = false;
			_entity_tf.mouseEnabled = false;
			_btm_bar.addChild(_entity_tf);
			
			// 骨骼计算次数
			var skel_label_tf : TextField = new TextField;
			skel_label_tf.defaultTextFormat = _label_format;
			skel_label_tf.autoSize = TextFieldAutoSize.LEFT;
			skel_label_tf.text = 'skel:';
			skel_label_tf.x = 10;
			skel_label_tf.y = _MID6_Y;
			skel_label_tf.selectable = false;
			skel_label_tf.mouseEnabled = false;
			_btm_bar.addChild(skel_label_tf);
			
			_skelCalc_tf = new TextField;
			_skelCalc_tf.defaultTextFormat = _data_format;
			_skelCalc_tf.autoSize = TextFieldAutoSize.LEFT;
			_skelCalc_tf.x = skel_label_tf.x + 45;
			_skelCalc_tf.y = _MID6_Y;
			_skelCalc_tf.selectable = false;
			_skelCalc_tf.mouseEnabled = false;
			_btm_bar.addChild(_skelCalc_tf);
		}
...
//				_poly_tf.text = _rfaces.toString().concat(' / ', _tfaces); // TODO: Total faces not yet available in 4.x
				...
				_particle_tf.text = _particleNum.toString();		// 粒子数				
				_drawCall_tf.text = _drawNum.toString() + " " + _drawUINum.toString();		// drawCall数
				_upload_tf.text = Context3DProxy._vbUploadCount.toString()+" "+Context3DProxy._ibUploadCount.toString()+" "+Context3DProxy._bmpUploadCount;
				_entity_tf.text = _numEntities.toString()
								+" "+_numOpaques.toString()
								+" "+_numBlended.toString()
								+" "+_numLights.toString()
								+" "+_numEditor.toString();
				
				_skelCalc_tf.text = _skelCalcNum.toString();
		...
		private function _onTimer(ev : Event) : void
		{
			...
			_particleNum = 0;
			_tfaces = _rfaces = 0;
			_drawNum = _drawUINum = 0;
			_numEntities = _numOpaques = _numBlended = _numLights = _numEditor = 0;
			_skelCalcNum = 0;
			...
			if (_views.length > 0)
			{
				...
				_drawNum = Context3DProxy.drawCall;			// 统计drawCall数
				_drawUINum = Context3DProxy.drawUICall;		// 统计3d ui drawCall数
				_skelCalcNum = SkeletonAnimator.calcTimes;	// 统计骨骼动画计算 
				...
				for (i=0; i<_views.length; i++)
				{
					...
					_particleNum += _views[i].entityCollector.numParticles;
					
					_numEntities += _views[i].entityCollector.numEntities;
					_numOpaques += _views[i].entityCollector.numOpaques;
					_numBlended += _views[i].entityCollector.numBlended;
					_numLights += _views[i].entityCollector.numTexLights;
					_numEditor += _views[i].entityCollector.numEditors;
				}
			}
		...
		public function stop(b:Boolean):void
		{
			...
			{
				_timer.stop();
				removeEventListener('timer', _onTimer);
				removeEventListener(Event.ENTER_FRAME, _onEnterFrame);
			}
			else
			{
				_timer.start();
				addEventListener('timer', _onTimer);
				addEventListener(Event.ENTER_FRAME, _onEnterFrame);
			}
		}
	}
