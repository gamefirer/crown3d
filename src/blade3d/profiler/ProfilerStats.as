package blade3d.profiler
{
	...
	public class ProfilerStats extends Sprite
	{
		...
		private var _averageFrameTime : Number = 0;
		...
		public function render(curTime : int, deltaTime : int) : void
		{
			...
			_averageFrameTime = _averageFrameTime * 0.9 + deltaTime * 0.1;
...
//					info += node.name + "=" + node.time + " " + node._reEntry;
			...
			// 每间隔一段时间更新函数信息
			if(_curTime - _lastFunctionInfoTime > 1000)
				_functionInfoDirty = true;
			
			if(_functionInfoDirty)
			{
				reDrawFunctionInfo();
				_lastFunctionInfoTime = _curTime;
		...
		private function reDrawFunctionInfo() : void
		{
			...
			else
			{
				...
				if(ni < _PROF_COUNT)
					_functionNames[ni].text = "frameTime " + _averageFrameTime.toFixed(1);
			}
		}
