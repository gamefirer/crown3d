package a3dparticle.animators.actions.uv
{
	 ...
	 * @author liaocheng
	 */
	public class UVDriftGlobal extends AllParticleAction
	{
		...
		public function UVDriftGlobal(cycle:Number,scale:Number=1, axis:int=U_AXIS)
		{
			priority = ParticleAnimationSet.POST_PRIORITY + 5;
			
			if (scale != 1)_isScale = true;
			_axis = axis;
			_data = Vector.<Number>([Math.PI * 2 / cycle, scale, 0, 0]);
		}
		
		override public function set animation(value:ParticleAnimationSet):void
		{
		...
		override public function getAGALVertexCode(pass:MaterialPassBase):String
		{
			if (_animation.needUV)
			{
				...
				var sin:ShaderRegisterElement = shaderRegisterCache.getFreeVertexSingleTemp();
				
				var code:String = "";
				
				if (_isScale) code += "mul " + target.toString() + "," + target.toString() + "," + uvParamConst.toString() + ".y\n";
				code += "mul " + sin.toString() + "," + _animation.vertexTime.toString() + "," + uvParamConst.toString() + ".x\n";
