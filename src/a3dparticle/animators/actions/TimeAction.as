package a3dparticle.animators.actions 
{
	...
	public class TimeAction extends PerParticleAction
	{
		...
		{
			getExtraData(subContainer).push(_tempStartTime);				// 粒子起始时间
			getExtraData(subContainer).push(_tempEndTime);					// 粒子持续时间
			getExtraData(subContainer).push(_tempSleepTime + _tempEndTime);	/// 持续+休眠时间
			getExtraData(subContainer).push(1 / _tempEndTime);				// 1/粒子持续时间
		}
		
		override public function getAGALVertexCode(pass : MaterialPassBase) : String
		{
			timeAtt = shaderRegisterCache.getFreeVertexAttribute();//timeAtt.x 开始时间，timeAtt.y 持续时间, timeAtt.y 持续+休眠时间
