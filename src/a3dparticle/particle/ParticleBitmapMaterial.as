package a3dparticle.particle
{
	...
	public class ParticleBitmapMaterial extends ParticleMaterialBase
	{
		...
		public function ParticleBitmapMaterial(bitmap:BitmapData, smooth:Boolean = true, repeat : Boolean = false, mipmap : Boolean = true, alphaThreshold:Number = 0)
		{
			...
			this._repeat = repeat;
			this._mipmap = mipmap;
			_texture = BitmapTextureCache.instance().getTexture(bitmap);
			_cutOffData = new Vector.<Number>(4, true);
			
			if (alphaThreshold < 0)
				alphaThreshold = 0;
            else if (alphaThreshold > 1) 
				alphaThreshold = 1;
			_alphaThreshold = alphaThreshold;
            _cutOffData[0] = _alphaThreshold;
		}
		
		override public function initAnimation(particleAnimation:ParticleAnimationSet):void
		{
		...
		override public function getFragmentCode(_particleAnimation:ParticleAnimationSet):String
		{
		...
		override public function render(_particleAnimation:ParticleAnimationSet, renderable : IRenderable, stage3DProxy : Stage3DProxy, camera : Camera3D) : void
		{
		...
		{
			BitmapTextureCache.instance().freeTexture(_texture);
			_texture = null;
			super.dispose();
		}
