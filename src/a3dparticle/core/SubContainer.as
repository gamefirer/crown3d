package a3dparticle.core
{
	...
	public class SubContainer implements IRenderable
	{
		...
		public function getVertexBuffer(stage3DProxy : Stage3DProxy) : VertexBuffer3D
		{
			...
			{
				_shareAtt._vertexBuffer = Context3DProxy.createVertexBuffer(_shareAtt._vertices.length/3, 3);
				Context3DProxy.uploadVertexBufferFromVector(_shareAtt._vertexBuffer, _shareAtt._vertices, 0, _shareAtt._vertices.length / 3);
				_shareAtt._vertexContex3D = Context3DProxy.context3D;
			}
		...
		public function getIndexBuffer(stage3DProxy : Stage3DProxy) : IndexBuffer3D
		{
			...
			{
				_shareAtt._indexBuffer = Context3DProxy.createIndexBuffer(_shareAtt._indices.length);
				Context3DProxy.uploadIndexBufferFromVector(_shareAtt._indexBuffer, _shareAtt._indices, 0, _shareAtt._indices.length);
				_shareAtt._indexContex3D = Context3DProxy.context3D;
			}
		...
		public function getUVBuffer(stage3DProxy : Stage3DProxy) : VertexBuffer3D
		{
			...
			{
				_shareAtt._uvBuffer = Context3DProxy.createVertexBuffer(_shareAtt._uvData.length/2, 2);
				Context3DProxy.uploadVertexBufferFromVector(_shareAtt._uvBuffer, _shareAtt._uvData, 0, _shareAtt._uvData.length / 2);
				_shareAtt._uvContex3D = Context3DProxy.context3D;
			}
		...
		{
			Context3DProxy.disposeVertexBuffer(_shareAtt._vertexBuffer);
			Context3DProxy.disposeIndexBuffer(_shareAtt._indexBuffer);
			Context3DProxy.disposeVertexBuffer(_shareAtt._uvBuffer);
			_material.dispose();
			_material = null;
		}
